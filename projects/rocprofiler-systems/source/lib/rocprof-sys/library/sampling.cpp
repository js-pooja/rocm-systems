        ...
        if(_signal_types->count(get_sampling_overflow_signal()) > 0)
        {
            ...
            _sampler->configure(overflow{
                ...
                [](int, pid_t, long, std::int64_t) {
                    // set_ready_signal() already ran once on the owning thread
                    // at the explicit call above. overflow::start() re-invokes
                    // this initer on every start() including resume, which
                    // runs on the time_window worker thread - redoing
                    // F_SETOWN there would rebind signal delivery to a thread
