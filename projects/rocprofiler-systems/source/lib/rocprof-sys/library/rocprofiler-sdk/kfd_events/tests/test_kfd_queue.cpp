...
#include "library/rocprofiler-sdk/kfd_events/kfd_queue.hpp"
...
// Self-contained stand-in for SdkBackend: kfd_queue<SdkBackend> and
// on_kfd_queue<SdkBackend> only ever touch these four members.
struct mock_sdk
{
    struct context_id_t
    {
...
TEST(kfd_queue_test, descriptor_reports_correct_metadata)
{
    using mock_dispatcher =
        buffered_callback_dispatcher<mock_sdk,
                                     ...
                                     on_kfd_queue<mock_sdk>>;
    constexpr const auto& domain = kfd_queue<mock_sdk>;
...
TEST(kfd_queue_test, on_kfd_queue_handles_empty_record_batch_without_crashing)
{
    ...
    on_kfd_queue<mock_sdk>(&record, nullptr);
}
