...
// Self-contained stand-in for SdkBackend: kfd_page_migrate<SdkBackend, Externals> and
// on_kfd_page_migrate<SdkBackend, Externals> only ever touch these members.
struct mock_sdk
{
    struct context_id_t
    {
...
// on_kfd_page_migrate<mock_sdk, externals> touches through Externals.
struct agent_t
{
    int         type              = 0;
...
// Externals mirrors the real ExternalDeps policy surface used by on_kfd_page_migrate
// and on_kfd_page_migrate_configure with no-op bodies -- these tests only assert the
