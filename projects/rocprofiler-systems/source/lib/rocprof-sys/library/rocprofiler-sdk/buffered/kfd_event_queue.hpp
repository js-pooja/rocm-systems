...
{
    Externals::add_string(Externals::kfd_event_queue_category_name);

    auto& agent_mgr  = Externals::get_agent_manager();
    auto  gpu_agents = agent_mgr.get_agents_by_type(Externals::AGENT_TYPE_GPU);
    if(gpu_agents.empty())
    {
        LOG_DEBUG("kfd_event_queue: no GPU agents found; no PMC info will be registered");
    }
    for(const auto& gpu : gpu_agents)
    {
        ...
        Externals::add_pmc_info(typename Externals::pmc_info_t{
            ...
            .name             = std::string{ Externals::kfd_event_queue_category_name },
            .symbol           = "KFD Event Queue Operations",
            .description = std::string{ Externals::kfd_event_queue_category_description },
...
on_kfd_event_queue(typename SdkBackend::kfd_event_queue_record* record, void* data)
{
    ...
    {
        LOG_DEBUG("kfd_event_queue: agent lookup failed for handle {} ({})",
