...
// on_kfd_event_queue<mock_sdk, externals> touches through Externals.
struct agent_t
{
    int         type              = 0;
...
// Externals mirrors the real ExternalDeps policy surface used by on_kfd_event_queue
// and on_kfd_event_queue_configure with no-op bodies -- these tests only assert the
