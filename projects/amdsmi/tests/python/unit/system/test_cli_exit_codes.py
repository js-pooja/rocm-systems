...
# The CLI's _version.py is generated by CMake, so a source checkout carries only
# the .in template and amdsmi_parser's ``from _version import __version__`` dies.
# No exit-code test reads the value.
if importlib.util.find_spec("_version") is None:
    _stubs["_version"] = fake_module("_version", __version__="0.0.0+source")
